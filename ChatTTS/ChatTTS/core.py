...
# 在文件顶部添加（在所有函数定义之前）
# torch.serialization.add_safe_globals([BertTokenizerFast, AddedToken, Tokenizer, Model])
...
class Chat:
    ...
    def load_models(self, source='huggingface', force_redownload=False, local_path='<LOCAL_PATH>', **kwargs):
        if source == 'huggingface':
            hf_home = os.getenv('HF_HOME', os.path.expanduser("~/.cache/huggingface")) # modify by zsh
            # hf_home = os.path.join(os.getcwd(), "weight")
        ...
        self.logger.log(logging.INFO, f'download_path: {download_path}')
