...
def refine_text(
    ...
    **kwargs
    ...
    # # 定义要提取的特殊token前缀/列表
    # special_tokens = {
    #     '[Sbreak]', '[Pbreak]', '[break_0]',
    #     '[CLS]', '[SEP]', '[PAD]', '[UNK]', '[MASK]'  # 常见BERT特殊token
    # }

    # # 添加所有方括号包裹的token（如[xxx]格式）
    # special_tokens.update([t for t in vocab.keys() if t.startswith('[') and t.endswith(']')])

    # # 保存到文件
    # with open('special_tokens.txt', 'w', encoding='utf-8') as f:
    #     f.write("特殊 Token 列表:\n")
    #     for token in sorted(special_tokens):
    #         if token in vocab:  # 确保token存在
    #             f.write(f"Token: {token}, ID: {vocab[token]}\n")
